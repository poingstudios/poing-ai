...
from poing_reviewer.ai.openai_compatible import sanitize_ai_json_output
    ...
    def generate_review(
        ...
        for model in models:
            ...
            raw = self._call_model(
                prompt=prompt,
                model_name=model,
                system_prompt="You are Poing Reviewer. Return a JSON object with verdict (APPROVED, APPROVED_WITH_SUGGESTIONS, CHANGES_REQUESTED), summary, findings, and comments.",
                temperature=0.2,
                require_json=True,
            )
            if not raw:
